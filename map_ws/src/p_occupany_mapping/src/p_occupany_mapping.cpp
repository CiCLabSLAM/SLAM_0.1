#include "p_occupany_mapping.h"
#include "nav_msgs/GetMap.h"
#include "sensor_msgs/PointCloud.h"
#include "sensor_msgs/PointCloud2.h"
#include "geometry_msgs/Point32.h"
#include "geometry_msgs/PoseArray.h"
//统计滤波需要的头文件 用pcl自带的来做
...
//统计滤波
std::vector <double> FilterPoint(const geometry_msgs::PoseArray& kf_and_pts,int size)
{
  ...
  {
    cloud->points[i].x=kf_and_pts.poses[2+i].position.x;
    cloud->points[i].y=kf_and_pts.poses[2+i].position.y;
    cloud->points[i].z=kf_and_pts.poses[2+i].position.z;
  }
  ...
  {
     double point_x = (cloud_filtered->points[i].x) * 3;
     double point_z = (cloud_filtered->points[i].z) * 3;
     mapPointPoses.push_back(point_x);
     mapPointPoses.push_back(point_z);
  }
  return mapPointPoses;
}
...
void callback(const geometry_msgs::PoseArray& kf_and_pts)
{
     ...
     mapPointPoses=FilterPoint(kf_and_pts,size);
     int cnt=mapPointPoses.size()/2;
     OccupanyMapping(mapPointPoses,cam_x,cam_z,cnt);
